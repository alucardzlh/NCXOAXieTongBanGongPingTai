...
import com.OA.Entity.MyRecApply_xzswyy_Bean;
 ...
 *
 */
public abstract class RecApplylistYYManager {
	private String TAG="NOT";
	
	public void getRecBrowseList(final Context paramActivity, final int paramInt,final String UserCode,
		...
		BaseAsyncTask loacl =new BaseAsyncTask(paramActivity,true){
			...
			MyRecApply_xzswyy_Bean bean;
			@Override
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				try {
					result=downloadDBB(paramActivity,"GetRecBrowseList", url, UserCode, SignCode, PageSize, PageNow, Status, Title, 
							LaiWenCompany, WenHao, SendStartTime, SendEndTime);
					Log.i(TAG, "我得收文列表===============" + result);
					if(result.contains("\"totalcount\":\"0\"")){
						bean = new MyRecApply_xzswyy_Bean();
						bean.setTotalcount("0");
						return Integer.valueOf(0);
					}
					Gson gson = new Gson();
					java.lang.reflect.Type type = new TypeToken<MyRecApply_xzswyy_Bean>() {}.getType();
			...
			protected void onPostExecute(Integer paramInteger) {
				...
				if (paramInteger == 1) {// 获取到返回的信息
					if (bean != null) {	
						...
						Constants.list_recApply_xzswyy_bean = bean.getRows();
						Constants.COUNT_OF_LIST_MyRECAPPLY_XZSWYY_BEAN = bean.getRows().size();
					}
				...
				RecApplylistYYManager.this.handlerLoginInfo(paramActivity,
