 ...
 * 会议室审核
 */
public abstract class Meeting_hyssh_ListManager {
private String TAG="Meeting_hyssh_ListManager";
	 ...
	 */
	public void getMeeting_hyssh_ListManager(final Context paramActivity,final int paramInt,
		...
		BaseAsyncTask loacl =new BaseAsyncTask(paramActivity,true){
			MyMeeting_hyssh_Bean bean;
			String result=null; //从服务器获取xml数据
			@Override
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				try {
					result=downloadDB(paramActivity,"GetMeetingCheckList", url, PageSize, PageNow, UserCode, SignCode, RoomName, StarTime, EndTime);
					JSONObject jsonObject = new JSONObject(result);
					String str = jsonObject.getString("totalcount");
					if(str.equals("0")){
						bean = new MyMeeting_hyssh_Bean();
						bean.setTotalcount("0");
						return Integer.valueOf(0);
					}
					Gson gson = new Gson();
					java.lang.reflect.Type type = new TypeToken<MyMeeting_hyssh_Bean>() {
					}.getType();
			...
			protected void onPostExecute(Integer paramInteger) {
				...
				if (paramInteger == 1) {// 获取到返回的信息
					if (bean != null) {
						...
						Constants.list_meeting_hyssh_bean = bean.getRows();
				...
				Meeting_hyssh_ListManager.this.handlerLoginInfo(paramActivity,
