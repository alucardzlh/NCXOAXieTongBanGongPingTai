 ...
 *补报参会人员列表
 */
public abstract class AddMtingWorkerListManager {
	 ...
	 * 
	 */
	public void getAddMtingWorkerListManager(final Context paramActivity,final int paramInt ,final int RoomLogID,final int DeptID,final String UserCode,final String  SignCode){
//		final String url ="http://117.40.244.140/WebServices/AppService.asmx?WSDL";
		...
		BaseAsyncTask loacl =new BaseAsyncTask(paramActivity,true){
			MyMeetingDetail_bbchry_Bean bean;
			String result=null; //从服务器获取xml数据
			@Override
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				try {
				
					result=downloadDB("AddMtingWorkerList",url, RoomLogID, DeptID, UserCode, SignCode);
					JSONObject jsonObject = new JSONObject(result);
					String str = jsonObject.getString("status");
					Gson gson = new Gson();
					java.lang.reflect.Type type = new TypeToken<MyMeetingDetail_bbchry_Bean>() {
					}.getType();
			...
			protected void onPostExecute(Integer paramInteger) {
				...
				if (paramInteger == 1) {// 获取到返回的信息
					if (bean != null) {
						...
						Constants.mymeetingdetail_bbchry_bean = bean;
					}
				...
				AddMtingWorkerListManager.this.handlerLoginInfo(paramActivity,
