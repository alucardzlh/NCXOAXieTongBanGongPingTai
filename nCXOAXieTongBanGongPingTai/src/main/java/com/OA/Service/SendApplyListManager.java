 ...
 * 发文申请列表
 */
public abstract class SendApplyListManager {
	 ...
	 */
	public void GetSendApplyListManager(final Context paramActivity,final int paramInt,final String UserCode,final String SignCode,final int RoleID,final int KindID){
//		final String url ="http://ncxceshioa.dichuang.cc/WebServices/AppService.asmx?WSDL";
		...
		BaseAsyncTask loacl =new BaseAsyncTask(paramActivity,true){
			...
			private List<MyXzfwzg_Bean> list;
			@Override
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				try {
					...
					Type listType = new TypeToken<ArrayList<MyXzfwzg_Bean>>(){}.getType();  
			        Gson gson = new Gson();  
			        list = gson.fromJson(result, listType);  
			        Constants.lis_xzfw_xzwfzg_bean=list;
...
//				 判断返回值，为空或者字符串“servicefail”返回-1.否则返回1
			...
			protected void onPostExecute(Integer paramInteger) {
				...
				SendApplyListManager.this.handlerLoginInfo(paramActivity,
