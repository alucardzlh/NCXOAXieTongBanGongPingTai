 ...
 * 我的发文列表
 */
public abstract class MySendApplyListManager {
private String TAG="NOT";
	 ...
	 */
	public void getMySendApplyListManager(final Context paramActivity,final int paramInt,
		...
		BaseAsyncTask loacl =new BaseAsyncTask(paramActivity,true){
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				try {
					...
					result=downloadDB("GetMySendApplyList", url, UserCode, SignCode, PageSize, PageNow, Status, Title, WenHao, NiGaoCompany, SendStartTime, SendEndTime);
					if(result.contains("\"totalcount\":\"0\"")){
						bean = new MyXzfw_Bean();
						bean.setTotalcount("0");
						return Integer.valueOf(0);
					}
					Gson gson = new Gson();
					java.lang.reflect.Type type = new TypeToken<MyXzfw_Bean>() {
					}.getType();
			...
			protected void onPostExecute(Integer paramInteger) {
				...
				if (paramInteger == 1) {// 获取到返回的信息
					if (bean != null) {
						...
						Constants.list_wdxzfw_bean =bean.getRows();
				...
				MySendApplyListManager.this.handlerLoginInfo(paramActivity,
