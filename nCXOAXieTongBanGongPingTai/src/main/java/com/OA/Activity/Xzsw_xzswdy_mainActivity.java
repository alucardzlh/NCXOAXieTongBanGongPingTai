...
import com.OA.Adapter.RecApply_Xzswdy_Adapter;
...
import com.OA.Service.RecApplyListDYManager;
...
public class Xzsw_xzswdy_mainActivity extends BaseActivity implements OnClickListener{
	...
	ListView lv_base;
	List<RecApply_xzswdy_Bean> list = new ArrayList<RecApply_xzswdy_Bean>();
	...
	RecApply_Xzswdy_Adapter adapter;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		// TODO 自动生成的方法存根
		super.onCreate(savedInstanceState);
		setContentView(R.layout.xzsw_xzswdy_main);
		initViewAndListener();
		myMeeting.getRecBrowseList1(Xzsw_xzswdy_mainActivity.this, 1,infoFile_.infoUsername().get(), 
...
private boolean loadfinish = true; // 指示数据是否加载完成
	...
	private RecApplyListDYManager myMeeting = new RecApplyListDYManager() {
		...
		protected void handlerLoginInfo(Context context, HandleResult handleResult,
			...
			if(handleResult.getiSuccess() != null && handleResult.getiSuccess().equals("success")){
				Count += Constants.COUNT_OF_LIST_MyRECAPPLY_XZSWDY_BEAN;
				if (lv_base.getAdapter() == null) {
					list = Constants.list_recApply_xzswdy_bean;
					if(list.size() == 0){
						return;
					}
					adapter = new RecApply_Xzswdy_Adapter(Xzsw_xzswdy_mainActivity.this,list);
					addFoot();
					...
					list.addAll(Constants.list_recApply_xzswdy_bean);
				...
				list = new ArrayList<RecApply_xzswdy_Bean>();
				adapter = new RecApply_Xzswdy_Adapter(Xzsw_xzswdy_mainActivity.this,list);
				lv_base.setAdapter(adapter);
			}
			tishiInfo(Constants.COUNT_OF_LIST_MyRECAPPLY_XZSWDY_BEAN,Count);
		}
	};
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.back:
			Xzsw_xzswdy_mainActivity.this.finish();
			break;
