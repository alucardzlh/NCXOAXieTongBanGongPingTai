 ...
 * 
 */
public class Xzfw_Wdxzfw_MainActivity extends BaseActivity implements
		OnClickListener {
	InfoFile_ infoFile_;
	LinearLayout back;
	...
	ListView lv_base;
	List<Wdxzfw_Bean> list = new ArrayList<Wdxzfw_Bean>();
	...
	MyXzfw_Wdxzfw_Adapter adapter;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.xzfw_wdxzfw_main);
		initViewAndListener();
//		mySend.getMySendApplyListManager(Xzfw_Wdxzfw_MainActivity.this, 1,"admin", "af8a15d2c3085fb3d818179b13a7eb29", 
//							10, indexPager, "","", "", "", "", "");
		mySend.getMySendApplyListManager(Xzfw_Wdxzfw_MainActivity.this, 1,infoFile_.infoUsername().get(), 
			...
			if ((lastItemid + 1) == loadtotal) { // 达到数据的最后一条记录
				...
				if (loadtotal > 0) {
					...
					if (loadfinish && isMore) {
						loadfinish = false;
							mySend.getMySendApplyListManager(Xzfw_Wdxzfw_MainActivity.this, 1, infoFile_.infoUsername().get(),infoFile_.infoUserType().get(),  
									10, nextpage, "",title,wenHao, niGaoCompany, sendStartTime, sendEndTime);
						}
				}
			}
		}
	...
	private MySendApplyListManager mySend = new MySendApplyListManager() {
		...
		protected void handlerLoginInfo(Context context, HandleResult handleResult,
			...
			if(handleResult.getiSuccess() != null && handleResult.getiSuccess().equals("success")){
				...
				if (lv_base.getAdapter() == null) {
					list = Constants.list_wdxzfw_bean;
					if(list.size() == 0){
						return;
					}
					adapter = new MyXzfw_Wdxzfw_Adapter(Xzfw_Wdxzfw_MainActivity.this,list);
					addFoot();
					...
					if(isSearchButtonClickedFirst){
						...
						list.addAll(Constants.list_wdxzfw_bean);
				...
				list = new ArrayList<Wdxzfw_Bean>();
				adapter = new MyXzfw_Wdxzfw_Adapter(Xzfw_Wdxzfw_MainActivity.this,list);
				lv_base.setAdapter(adapter);
			}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.back:
			Xzfw_Wdxzfw_MainActivity.this.finish();
			break;
