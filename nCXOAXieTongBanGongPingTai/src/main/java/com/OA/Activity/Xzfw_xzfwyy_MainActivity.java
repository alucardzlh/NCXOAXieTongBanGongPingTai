 ...
 * 
 */
public class Xzfw_xzfwyy_MainActivity extends BaseActivity implements
		OnClickListener {
	InfoFile_ infoFile_;
	LinearLayout back;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.xzfw_xzfwyy_main);
		initViewAndListener();
		endBrows.GetSendBrowseListManager(Xzfw_xzfwyy_MainActivity.this, 1,infoFile_.infoUsername().get(), infoFile_.infoUserType().get(), 
				10, indexPager, "200",title,wenHao, niGaoCompany, sendStartTime, sendEndTime);
			...
			if ((lastItemid + 1) == loadtotal) { // 达到数据的最后一条记录
				...
				if (loadtotal > 0) {
					...
					if (loadfinish && isMore) {
						loadfinish = false;
						endBrows.GetSendBrowseListManager(Xzfw_xzfwyy_MainActivity.this, 1, infoFile_.infoUsername().get(),infoFile_.infoUserType().get(), 
									10, nextpage, "200",title,wenHao, niGaoCompany, sendStartTime, sendEndTime);
						}
				}
			}
		}
		...
		protected void handlerLoginInfo(Context context, HandleResult handleResult,
			...
			if(handleResult.getiSuccess() != null && handleResult.getiSuccess().equals("success")){
				...
				if (lv_base.getAdapter() == null) {
					...
					adapter = new MyXzfw_Xzfwyy_Adapter(Xzfw_xzfwyy_MainActivity.this,list);
					addFoot();
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.back:
			Xzfw_xzfwyy_MainActivity.this.finish();
			break;
