...
public class Xzsw_Detail_BDxx_MainActivity extends BaseActivity implements
		OnClickListener {
	...
	ListView lv_base;
	private LinearLayout ll_form,view_checkPeople;;
	private LinearLayout bdfoot;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.xzsw_xzswdj_btbd);
		initViewAndListener();
		...
		getFlowFormElements.getFlowFormElements(
				Xzsw_Detail_BDxx_MainActivity.this, 1, infoFile_.infoUsername()
						.get(), infoFile_.infoUserType().get(), Integer
						.valueOf(FlowID), 0, -1);
	}
	...
	private void initViewAndListener() {
		...
		bdtou = (LinearLayout) LayoutInflater.from(this).inflate(
				R.layout.xzsw_bdxx_item_dbtou, null);
		bdfoot = (LinearLayout) LayoutInflater.from(this).inflate(
				R.layout.xzsw_bdxx_item_bdfoot, null);
		 ...
		 * lv_base.addHeaderView(bdtou); lv_base.addFooterView(bdfoot);
		 */
		...
		edt_tgbt.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				final EditText inputServer = new EditText(
						Xzsw_Detail_BDxx_MainActivity.this);
				...
				builder.setPositiveButton("确定",
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int which) {
								edt_tgbt.setText(inputServer.getText()
										.toString());
							}
						});
				builder.show();
			...
			if (handleResult.getiSuccess() != null
				...
				case 1:// 获取表单接口
					...
					if(handleResult.isSelPeople()){
						...
						ll_checkpeople.setOnClickListener(new OnClickListener() {
							...
							@Override
							public void onClick(View v) {
								Xzsw_Detail_BDxx_MainActivity.this.startActivityForResult(new Intent(Xzsw_Detail_BDxx_MainActivity.this,
					...
					btn_fs = (Button) findViewById(R.id.btn_fs);
					btn_qczw = (Button) findViewById(R.id.btn_qczw);
					btn_fs.setOnClickListener(Xzsw_Detail_BDxx_MainActivity.this);
					btn_qczw.setOnClickListener(Xzsw_Detail_BDxx_MainActivity.this);
					break;
				case 2:// 加载正文范文
				...
				case 3:// 保存正文成功，开始提交步骤第一步
					...
					getFlowFormElements.GetApplyNextStepInfo(
							Xzsw_Detail_BDxx_MainActivity.this, 4, Integer
									.valueOf(FlowID).intValue(), -1, infoFile_
									.infoUsername().get());
					break;
				case 4:// 开始提交步骤第二步
					datas = handleResult.getDatas();
					infoFile_.edit().TypeOfLast().put(4000).apply();
					if (!datas.getFlowAction().equals("BranchSel")) {
						Intent intent = new Intent();
						intent.setClass(Xzsw_Detail_BDxx_MainActivity.this,
	...
	GetShouWenFormElementsManager getShouWenFormElementsManager = new GetShouWenFormElementsManager() {
	...
	public void onClick(View v) {
		...
		case R.id.imageView3:
			Xzsw_Detail_BDxx_MainActivity.this.finish();
			break;
		case R.id.ll1:
		...
		case R.id.btn_fs:
			if (!hasQCZW) {
				Toast.makeText(Xzsw_Detail_BDxx_MainActivity.this, "请先起草正文！",
						Toast.LENGTH_SHORT).show();
				return;
			}
			if (edt_tgbt.getText().toString().trim().equals("")) {
				Toast.makeText(Xzsw_Detail_BDxx_MainActivity.this, "请填写公文标题！",
						Toast.LENGTH_SHORT).show();
				return;
			}
			if (tv_1.getText().toString().trim().equals("")) {
				Toast.makeText(Xzsw_Detail_BDxx_MainActivity.this, "请填写截止日期！",
						Toast.LENGTH_SHORT).show();
				return;
			}
			...
			for (int i = 0; i < list.size(); i++) {
				if (list.get(i).getElementBlank().equals("0")
						&& list.get(i).getElementValue().equals("")) {
					Toast.makeText(Xzsw_Detail_BDxx_MainActivity.this,
							list.get(i).getElementName() + "不能为空！！",
							Toast.LENGTH_SHORT).show();
					return;
				}
			}
			...
			getFlowFormElements.SaveArchiveData(
					Xzsw_Detail_BDxx_MainActivity.this, 3, 0, 0, 0, infoFile_
							.infoUsername().get(), infoFile_.infoUserType()
							.get(), str_zw_content);
			break;
		case R.id.btn_qczw:
			if (hasQCZW) {
				...
				/*
				 * List<String> list_value = null; List<String> list_name =
				 * null; for (int i = 0; i < list.size(); i++) { if
				 * (list.get(i).getElementName().equals("机关代字号")) { list_value =
				 * StringUtil.stringToNormalList(list.get(i) .getOptionValue());
				 * list_name = StringUtil.stringToNormalList(list.get(i)
				 * .getOptionsText()); if(list_name != null && list_value !=
				 * null){ for (int j = 0; j < list_name.size(); j++) { if
				 * (list.get(i).getElementValue() .equals(list_name.get(j))) {
				 * Tildes = Integer.valueOf(list_value.get(j)) .intValue(); } }
				 * }
				 * 
				 * } }
				 */
				getFlowFormElements.LoadArchiveData(
						Xzsw_Detail_BDxx_MainActivity.this, 2,
						Constants.tildes, ArcStepID, 0, 0);
			}
			break;
		default:
			break;
	...
	MyCallBack callBack = new MyCallBack() {
		...
		public void callback(String id, String type, String value, int position) {
			if (list.get(position).getElementType()
					.equals("MultipleFileUpload")
