 ...
 *
 */
public class Detail_Xzswcl_chakantabs_MainActivity extends FragmentActivity
	...
	private String Title, endTime, Memo;
	...
	private String tag;
	receive_swcl receive;

	@Override
	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		requestWindowFeature(Window.FEATURE_NO_TITLE);
		setContentView(R.layout.xzsw_xzswcl_tabs_main);
		...
		Log.i(tag, "---WFStepID----" + WFStepID);
		
		InitTextView();
		getDatasFromInterface();
		
		// 生成广播处理
		receive = new receive_swcl();
		// 实例化过滤器并设置要过滤的广播
	...
	private void getDatasFromInterface() {
				getFlowFormElements.getFlowFormElements(Detail_Xzswcl_chakantabs_MainActivity.this, 1, infofile
						.infoUsername().get(), infofile.infoUserType().get(),FlowID, WFID,Integer.valueOf(infofile.CurNode().get()).intValue());
	}
	...
	private void InitViewPager() {
		...
		Detail_Xzswcl_chakan_Fragment1 f1 = new Detail_Xzswcl_chakan_Fragment1()
	...
	public void onClick(View v) {
		...
		case R.id.btn_ty:
			...
			getFlowFormElements.GetCheckNextStepInfo(this, 1000, FlowID,
					infofile.WFID().get(),0, infofile.CurNode().get()+"", "1",0,
					infofile.infoUsername().get());
			break;
		case R.id.btn_bty:
			Constants.shouwenOrfawen = 4000;
			...
			Constants.userCode = cxry_usercode.toString();
			getFlowFormElements.GetCheckNextStepInfo(this, 1000, FlowID,
					infofile.WFID().get(), 0, infofile.CurNode().get()+"", "0",0,
					infofile.infoUsername().get());
			break;
		case R.id.btn_th:
			Memo = "";
		...
		case R.id.btn_ckzw:
			if (hasDownZW) {
				...
				getFlowFormElements.LoadArchiveData(
						Detail_Xzswcl_chakantabs_MainActivity.this, 2000, 0,
				...
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						backApply.getBackApplyManager(
								Detail_Xzswcl_chakantabs_MainActivity.this, 1,
								WFStepID, infofile.infoUsername().get(),
								infofile.infoUserType().get(), Memo);
					}
				})
	...
	GetFlowFormElementsManager getFlowFormElements = new GetFlowFormElementsManager() {
		...
		protected void handlerLoginInfo(Context context,
			...
			//			&& handleResult.getiSuccess().equals("success")) {
					getBrowseUser
							.getBrowseUser(
									Detail_Xzswcl_chakantabs_MainActivity.this,
									1, WFID);
			//	}
					if(handleResult.isSelPeople()){
						...
						ll_checkpeople1.setOnClickListener(new OnClickListener() {
							...
							@Override
							public void onClick(View v) {
								Detail_Xzswcl_chakantabs_MainActivity.this.startActivityForResult(new Intent(Detail_Xzswcl_chakantabs_MainActivity.this,
			...
			case 1000:
				if (handleResult.getiSuccess() != null
					...
					infoFile.edit().TypeOfLast().put(3000).apply();
	...
	public class receive_swcl extends BroadcastReceiver {
	...
	GetBrowsePeopleListManager getBrowseUser = new GetBrowsePeopleListManager() {
		...
		//			&& handleResult.getiSuccess().equals("success")) {
				...
				switch (paramInt) {
				case 1:
					getBrowseUser.getNotBrowseUser(Detail_Xzswcl_chakantabs_MainActivity.this, 2,WFID);
					break;
				case 2:
					getBrowseUser.getOverBrowseUser(Detail_Xzswcl_chakantabs_MainActivity.this, 3,WFID);
					break;
				case 3:
					InitViewPager();
					break;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == 3000 && resultCode == Activity.RESULT_OK) {
			for (int i = 0; i < Constants.list_right.size(); i++) {
				if(tv_llry.getText().toString().trim().contains(Constants.list_right.get(i).getUserName())){
					continue;
				}
				if (i == Constants.list_right.size() - 1) {
					if(tv_llry.getText().toString()!= null && !tv_llry.getText().toString().equals("")){
