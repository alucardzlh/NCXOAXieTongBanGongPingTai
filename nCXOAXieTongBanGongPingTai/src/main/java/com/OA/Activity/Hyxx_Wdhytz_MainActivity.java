...
import com.OA.Adapter.MyMeeting_Wdhysq_Adapter;
import com.OA.Adapter.MyMeeting_Wdhytz_Adapter;
 ...
 * 
 */
public class Hyxx_Wdhytz_MainActivity extends BaseActivity implements
		OnClickListener {
	InfoFile_ infoFile_;
	LinearLayout back;
	EditText edt;
	...
	ListView lv_base;
	List<Meeting_Bean> list = new ArrayList<Meeting_Bean>();
	...
	MyMeeting_Wdhytz_Adapter adapter;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.hyxx_wdhytz_main);
		initViewAndListener();
		myMeeting.getMeetingTZListManager(Hyxx_Wdhytz_MainActivity.this, 1, 10, indexPager,infoFile_.infoUsername().get(), infoFile_.infoUserType().get(), "", "", "");
	}
			...
			if ((lastItemid + 1) == loadtotal) { // 达到数据的最后一条记录
				...
				if (loadtotal > 0) {
					...
					if (loadfinish && isMore) {
						loadfinish = false;
							myMeeting.getMeetingTZListManager(Hyxx_Wdhytz_MainActivity.this, 1, 10, nextpage,infoFile_.infoUsername().get(), 
	...
	private MeetingListManager myMeeting = new MeetingListManager() {
		...
		protected void handlerLoginInfo(Context context, HandleResult handleResult,
			...
			if(handleResult.getiSuccess() != null && handleResult.getiSuccess().equals("success")){
				...
				if (lv_base.getAdapter() == null) {
					list = Constants.list_meeting_bean;
					if(list.size() == 0){
						return;
					}
					adapter = new MyMeeting_Wdhytz_Adapter(Hyxx_Wdhytz_MainActivity.this,list);
					addFoot();
					...
					if(isSearchButtonClickedFirst){
						...
						list.addAll(Constants.list_meeting_bean);
				...
				list = new ArrayList<Meeting_Bean>();
				adapter = new MyMeeting_Wdhytz_Adapter(Hyxx_Wdhytz_MainActivity.this,list);
				lv_base.setAdapter(adapter);
			}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.back:
			Hyxx_Wdhytz_MainActivity.this.finish();
			break;
		...
		case R.id.btn_search:
			...
			endTime=tv_1.getText().toString().trim();
			roomName = edt.getText().toString().trim();
			myMeeting.getMeetingTZListManager(Hyxx_Wdhytz_MainActivity.this, 1, 10, 1,infoFile_.infoUsername().get(), 
						infoFile_.infoUserType().get(), roomName,startTime,endTime);
			break;
		case R.id.ll1:
