 ...
 * 
 */
public class Hyxx_Wdhysq_MainActivity extends BaseActivity implements
		OnClickListener {
	InfoFile_ infoFile_;
	LinearLayout back;
	EditText edt;
	...
	MyMeeting_Wdhysq_Adapter adapter;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.hyxx_wdhysq_main);
		initViewAndListener();
		myMeeting.getMeetingListManager(Hyxx_Wdhysq_MainActivity.this, 1, 10, indexPager,infoFile_.infoUsername().get(), infoFile_.infoUserType().get(), "", "", "");
	}
			...
			if ((lastItemid + 1) == loadtotal) { // 达到数据的最后一条记录
				...
				if (loadtotal > 0) {
					...
					if (loadfinish && isMore) {
						loadfinish = false;
		//				if(isSearchModeList){
							myMeeting.getMeetingListManager(Hyxx_Wdhysq_MainActivity.this, 1, 10, nextpage,infoFile_.infoUsername().get(), 
		...
		protected void handlerLoginInfo(Context context, HandleResult handleResult,
			...
			if(handleResult.getiSuccess() != null && handleResult.getiSuccess().equals("success")){
				...
				if (lv_base.getAdapter() == null) {
					...
					adapter = new MyMeeting_Wdhysq_Adapter(Hyxx_Wdhysq_MainActivity.this,list);
					addFoot();
...
//				}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.back:
			Hyxx_Wdhysq_MainActivity.this.finish();
			break;
		...
		case R.id.btn_search:
			...
			myMeeting.getMeetingListManager(Hyxx_Wdhysq_MainActivity.this, 1, 10, 1,infoFile_.infoUsername().get(), 
						infoFile_.infoUserType().get(), roomName,startTime,endTime);
			break;
		case R.id.ll1:
