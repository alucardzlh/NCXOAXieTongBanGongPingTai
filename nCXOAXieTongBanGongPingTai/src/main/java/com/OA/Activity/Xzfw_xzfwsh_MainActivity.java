...
import com.OA.Adapter.MyXzfw_Xzfwsh_Adapter;
 ...
 * 
 */
public class Xzfw_xzfwsh_MainActivity extends BaseActivity implements
		OnClickListener {
	InfoFile_ infoFile_;
	LinearLayout back;
	...
	Button btn_search;
	ListView lv_base;
	List<Xzfwsh_Bean> list = new ArrayList<Xzfwsh_Bean>();
...
//	
	...
	String sendEndTime;
//	
	MyXzfw_Xzfwsh_Adapter adapter;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.xzfw_xzfwsh_main);
		initViewAndListener();
		sendhecklist.getSendCheckListManager(Xzfw_xzfwsh_MainActivity.this, 1,infoFile_.infoUsername().get(), infoFile_.infoUserType().get(), 
				10, indexPager,title,wenHao, niGaoCompany, sendStartTime, sendEndTime);
//		sendhecklist.getSendCheckListManager(Xzfw_xzfwsh_MainActivity.this, 1, "admin","af8a15d2c3085fb3d818179b13a7eb29", 
//				1, 1, "", "", "", "", "");
	}
...
//
	...
	private SendCheckListManager sendhecklist = new SendCheckListManager() {
		...
		protected void handlerLoginInfo(Context context, HandleResult handleResult,
			...
			if(handleResult.getiSuccess() != null && handleResult.getiSuccess().equals("success")){
				...
				if (lv_base.getAdapter() == null) {
					list = Constants.list_xzfwsh_bean;
					if(list.size() == 0){
						return;
					}
					adapter = new MyXzfw_Xzfwsh_Adapter(Xzfw_xzfwsh_MainActivity.this,list);
					addFoot();
					...
					if(isSearchButtonClickedFirst){
						...
						list.addAll(Constants.list_xzfwsh_bean);
				...
				list = new ArrayList<Xzfwsh_Bean>();
				adapter = new MyXzfw_Xzfwsh_Adapter(Xzfw_xzfwsh_MainActivity.this,list);
				lv_base.setAdapter(adapter);
			}
...
//		}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.back:
			Xzfw_xzfwsh_MainActivity.this.finish();
			break;
