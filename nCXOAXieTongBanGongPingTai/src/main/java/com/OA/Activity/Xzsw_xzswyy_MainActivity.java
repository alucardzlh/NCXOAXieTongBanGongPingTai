...
import com.OA.Adapter.RecApply_Xzswyy_Adapter;
...
import com.OA.Service.RecApplylistYYManager;
...
public class Xzsw_xzswyy_MainActivity extends BaseActivity implements OnClickListener{
	...
	ListView lv_base;
	List<RecApply_xzswyy_Bean> list = new ArrayList<RecApply_xzswyy_Bean>();
	...
	RecApply_Xzswyy_Adapter adapter;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		// TODO 自动生成的方法存根
		super.onCreate(savedInstanceState);
		setContentView(R.layout.xzsw_xzswyy_main);
		initViewAndListener();
	...
	private RecApplylistYYManager myMeeting = new RecApplylistYYManager() {
		...
		protected void handlerLoginInfo(Context context, HandleResult handleResult,
			...
			if(handleResult.getiSuccess() != null && handleResult.getiSuccess().equals("success")){
				Count += Constants.COUNT_OF_LIST_MyRECAPPLY_XZSWYY_BEAN;
				if (lv_base.getAdapter() == null) {
					list = Constants.list_recApply_xzswyy_bean;
					if(list.size() == 0){
						return;
					}
					adapter = new RecApply_Xzswyy_Adapter(Xzsw_xzswyy_MainActivity.this,list);
					addFoot();
					...
					list.addAll(Constants.list_recApply_xzswyy_bean);
				...
				list = new ArrayList<RecApply_xzswyy_Bean>();
				adapter = new RecApply_Xzswyy_Adapter(Xzsw_xzswyy_MainActivity.this,list);
				lv_base.setAdapter(adapter);
			}
			tishiInfo(Constants.COUNT_OF_LIST_MyRECAPPLY_XZSWYY_BEAN,Count);
		}
	};
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.back:
			Xzsw_xzswyy_MainActivity.this.finish();
			break;
		case R.id.btn_search:
			...
			/*List<RecApply_xzswyy_Bean> list = new ArrayList<RecApply_xzswyy_Bean>();
			Constants.list_recApply_xzswyy_bean = list;
			isSearchModeList = true;
			indexPager = 1;
			Constants.COUNT_OF_LIST_MyRECAPPLY_XZSWYY_BEAN = 0;
			Count = 0;
			if(!edt_bt.getText().toString().trim().equals("") && !edt_ngdw.getText().toString().trim().equals("")
					&& !edt_wh.getText().toString().trim().equals("")){
				title = edt_bt.getText().toString().trim();
				wenHao = edt_wh.getText().toString().trim();
				laiWenCompany = edt_ngdw.getText().toString().trim();
				myMeeting.getRecBrowseList(Xzsw_xzswyy_MainActivity.this, 1,infoFile_.infoUsername().get(), 
